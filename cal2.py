...
if dfs:
    ...
    if len(high_conf_df) > 0:
        ...
        total_high = len(high_conf_df)
        
        # 計算個別數量
        counts = high_conf_df['initial_decision'].value_counts()
        red_count = counts.get('Red', 0)
        black_count = counts.get('Black', 0)
        
        # 計算比例
        red_ratio = red_count / total_high
        black_ratio = black_count / total_high
        
        print(f"高信心樣本總數: {total_high}")
        ...
        group_stats = high_conf_df.groupby('group')['initial_decision'].value_counts(normalize=True).unstack().fillna(0)
        
        # 為了閱讀方便，轉成百分比格式顯示
        pd.options.display.float_format = '{:.2%}'.format
        print(group_stats)
    else:
        print("未發現任何信心大於 50 的數據。")
