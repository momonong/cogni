...
for i, h in enumerate(human_levels):
    for j, a in enumerate(AI_levels):
        ...
        stats = compute_compliance(subset)

        ax.bar(stats.index, stats[False],
               color='#e74c3c', alpha=0.8)
        ax.bar(stats.index, stats[True],
               bottom=stats[False],
...
plt.tight_layout()
plt.savefig("Fig1a_Trust_Gap_3x3.png", dpi=300, bbox_inches='tight')
plt.close()
